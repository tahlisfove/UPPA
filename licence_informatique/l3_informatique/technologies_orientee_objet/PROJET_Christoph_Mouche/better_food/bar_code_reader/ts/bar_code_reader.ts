...
let _bar_code: string;
let WebS: WebSocket;
let cb: any;

/* Connection of client-side WebSockets */
function connect(): void {
    if (WebS !== undefined && WebS.readyState !== WebSocket.CLOSED ) { return }
...
function send(_bar_code: string) { WebS.send (_bar_code) }
...
(window as any).DOM_ready.then(() => {
    /* Webcam shooting */
    const _camera = window.document.getElementById('camera') as HTMLVideoElement;
    _camera.addEventListener('canplay', () => {
        window.console.log("The video is playing...");
    });
    const _canvas = window.document.createElement('canvas') as HTMLCanvasElement;

    /* Camera constraints initialization */
    let constraints: {video: boolean | { facingMode: 'environement' } } = {video: true};
    ...
    const _video_management = () => {
        if (_camera.srcObject !== null) {
            ...
            if ((_camera.srcObject as MediaStream).getVideoTracks()[0].readyState === 'live') {
                ...
                _photo.onload = () => {
                    ...
                    (_camera.srcObject as MediaStream).getVideoTracks()[0].stop();
                    /* The stream is no longer active */
                    window.console.assert((_camera.srcObject as MediaStream).active === false);
                    window.console.assert((_camera.srcObject as MediaStream).getVideoTracks()[0].readyState === 'ended');
                ...
                function(result: any) {    
                    ...
                    if (result?.codeResult) {
                        let cb = window.document.getElementById('cb') as HTMLDivElement;
