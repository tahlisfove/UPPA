...
if (estNul(p)) if( estNul(p1)   &&  a0 == 0  ) success =1;
if(estNul(p1) && a0 == 0)   if(estNul(p) )   success = success +1;
...
if ( Degre(p) ==0 )    if ( estNul(p1)  )   success =1;
if(estNul(p1)   if ( Degre(p) ==0 )       success =  success + 1;

/* vérification de la propriété:    Degre(p) = Degre(p1)+1 estNul(p1) = False  */
if( Degre(p) =Degre(p1) +1  )  if (!estNul(p1) )         success =  success + 1;
if(!estNul(p1))   if( Degre(p)== Degre(p1) +1 )        success =  success + 1;
/* Bilan de la vérification */
if(success!=4) 
